...
def send_inline_chord_analysis(query: types.InlineQuery):
    logger.info(f"Inline chord analysis request from {message.from_user.username}")
    try:
...
def send_inline_compose_analysis(query: types.InlineQuery):
    logger.info(f"Inline compose analysis request from {message.from_user.username}")
    try:
